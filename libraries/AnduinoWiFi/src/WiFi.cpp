...
void AnduinoWiFi::setPins(int8_t cs, int8_t irq, int8_t rst, int8_t en)
{
	gi8Winc1501CsPin = cs;
...
char* AnduinoWiFi::firmwareVersion()
{
	tstrM2mRev rev;
...
uint8_t AnduinoWiFi::begin(const char *ssid)
{
	return startConnect(ssid, M2M_WIFI_SEC_OPEN, (void *)0);
}

uint8_t AnduinoWiFi::begin(const char *ssid, uint8_t key_idx, const char* key)
{
...
uint8_t AnduinoWiFi::begin(const char *ssid, const char *key)
{
	return startConnect(ssid, M2M_WIFI_SEC_WPA_PSK, key);
}

uint8_t AnduinoWiFi::startConnect(const char *ssid, uint8_t u8SecType, const void *pvAuthInfo)
{
	if (!_init) {
		init();
	}
...
uint8_t AnduinoWiFi::beginAP(const char *ssid)
{
	return beginAP(ssid, 1);
}

uint8_t AnduinoWiFi::beginAP(const char *ssid, uint8_t channel)
{
	return startAP(ssid, M2M_WIFI_SEC_OPEN, NULL, channel);
}

uint8_t AnduinoWiFi::beginAP(const char *ssid, uint8_t key_idx, const char* key)
{
	return beginAP(ssid, key_idx, key, 1);
}

uint8_t AnduinoWiFi::beginAP(const char *ssid, uint8_t key_idx, const char* key, uint8_t channel)
{
	tstrM2mWifiWepParams wep_params;
...
uint8_t AnduinoWiFi::startAP(const char *ssid, uint8_t u8SecType, const void *pvAuthInfo, uint8_t channel)
{
...
uint8_t AnduinoWiFi::beginProvision()
{
	return beginProvision(1);
}

uint8_t AnduinoWiFi::beginProvision(uint8_t channel)
{
...
uint8_t AnduinoWiFi::startProvision(const char *ssid, const char *url, uint8_t channel)
{
	tstrM2MAPConfig strM2MAPConfig;
...
uint32_t AnduinoWiFi::provisioned()
{
	m2m_wifi_handle_events(NULL);
...
void AnduinoWiFi::config(IPAddress local_ip)
{
	config(local_ip, (uint32_t)0);
}

void AnduinoWiFi::config(IPAddress local_ip, IPAddress dns_server)
{
	config(local_ip, dns_server, (uint32_t)0);
}

void AnduinoWiFi::config(IPAddress local_ip, IPAddress dns_server, IPAddress gateway)
{
	config(local_ip, dns_server, gateway, (uint32_t)0);
}

void AnduinoWiFi::config(IPAddress local_ip, IPAddress dns_server, IPAddress gateway, IPAddress subnet)
{
...
uint8_t *AnduinoWiFi::macAddress(uint8_t *mac)
{
...
uint32_t AnduinoWiFi::localIP()
{
	return _localip;
}

uint32_t AnduinoWiFi::subnetMask()
{
	return _submask;
}

uint32_t AnduinoWiFi::gatewayIP()
{
	return _gateway;
}

char* AnduinoWiFi::SSID()
{
...
uint8_t* AnduinoWiFi::BSSID(uint8_t* bssid)
{
	if (_mode == WL_AP_MODE) {
...
uint8_t* AnduinoWiFi::APClientMacAddress(uint8_t* mac)
{
	if (_mode == WL_AP_MODE) {
...
uint8_t* AnduinoWiFi::remoteMacAddress(uint8_t* remoteMacAddress)
{
...
int8_t AnduinoWiFi::scanNetworks()
{
...
char* AnduinoWiFi::SSID(uint8_t pos)
{
	wl_status_t tmp = _status;
...
int32_t AnduinoWiFi::RSSI(uint8_t pos)
{
	wl_status_t tmp = _status;
...
uint8_t AnduinoWiFi::encryptionType()
{ 
...
uint8_t AnduinoWiFi::encryptionType(uint8_t pos)
{
	wl_status_t tmp = _status;
...
uint8_t* AnduinoWiFi::BSSID(uint8_t pos, uint8_t* bssid)
{
	wl_status_t tmp = _status;
...
uint8_t AnduinoWiFi::channel(uint8_t pos)
{
	wl_status_t tmp = _status;
...
int AnduinoWiFi::hostByName(const char* aHostname, IPAddress& aResult)
{
...
void AnduinoWiFi::refresh(void)
{
	// Update state machine:
	m2m_wifi_handle_events(NULL);
}

void AnduinoWiFi::lowPowerMode(void)
{
	m2m_wifi_set_sleep_mode(M2M_PS_H_AUTOMATIC, true);
}

void AnduinoWiFi::maxLowPowerMode(void)
{
	m2m_wifi_set_sleep_mode(M2M_PS_DEEP_AUTOMATIC, true);
}

void AnduinoWiFi::noLowPowerMode(void)
{
	m2m_wifi_set_sleep_mode(M2M_NO_PS, false);
}

int AnduinoWiFi::ping(const char* hostname, uint8_t ttl)
{
	IPAddress ip;
...
int AnduinoWiFi::ping(const String &hostname, uint8_t ttl)
{
	return ping(hostname.c_str(), ttl);
}

int AnduinoWiFi::ping(IPAddress host, uint8_t ttl)
{
	// Network led ON (rev A then rev B).
	m2m_periph_gpio_set_val(M2M_PERIPH_GPIO16, 0);
	m2m_periph_gpio_set_val(M2M_PERIPH_GPIO5, 0);
