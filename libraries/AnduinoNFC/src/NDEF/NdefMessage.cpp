...
void NdefMessage::printPayload(uint8_t recordNum)
{  
    //initialize payload buffer for first record
    uint16_t payloadLength = _records[recordNum].getPayloadLength();
    byte payload[payloadLength];

        //print payload in HEX and ascii
        _records[recordNum].getPayload(payload);
        PrintHexChar(payload, _records[recordNum].getPayloadLength());
        Serial.println();

}

/*uint8_t NdefMessage::getPayloadLength(uint8_t recordNum)
{
    return _records[recordNum].getPayloadLength();
}*/
